...
public class PortfolioManagerApplication {
  ...
  public static List<String> mainReadQuotes(String[] args) throws IOException, URISyntaxException {
    ...
    File resource = resolveFileFromResources(args[0]);
    String endDate = args[1]; 
    List<String> stockQuotesList = new LinkedList<>();
    
    PortfolioTrade[] portfolioTrades = 
    getObjectMapper().readValue(resource, PortfolioTrade[].class);
    
    List<TotalReturnsDto> totalReturnsDtoList = getPortfolioTrade(portfolioTrades, endDate);
    //System.out.println(totalReturnsDtoList.size());
    ...
    for (TotalReturnsDto totalReturnsDto : totalReturnsDtoList) {
      stockQuotesList.add(totalReturnsDto.getSymbol());
    }
  ...
  public static List<String> mainReadFile(String[] args) throws IOException, URISyntaxException {
    ...
    // list.add(new String(Files.readAllBytes(resource.toPath())));

    PortfolioTrade[] portfolioTrades = 
    getObjectMapper().readValue(resource, PortfolioTrade[].class);

    for (PortfolioTrade portfolioTrade : portfolioTrades) {
      list.add(portfolioTrade.getSymbol());
    }
    return list;
  }
  ...
  public static List<String> debugOutputs() {
    ...
    String resultOfResolveFilePathArgs0 = 
            "/home/crio-user/workspace/officialkarmakar-ME_QMONEY/qmoney/bin/main/trades.json";
    String toStringOfObjectMapper = 
            "etObjectMapper().readValue(resource, PortfolioTrade[].class)";
    String functionNameFromTestFileInStackTrace = "mainReadFile(String[])";
    String lineNumberFromTestFileInStackTrace = "99";
  ...
  public static List<TotalReturnsDto> getPortfolioTrade(PortfolioTrade[] portfolioTrades, 
          String endDate) throws URISyntaxException {
    List<TotalReturnsDto> totalReturnsDtoList = new LinkedList<>();
    for (PortfolioTrade portfolioTrade : portfolioTrades) {

      String ticker = portfolioTrade.getSymbol();
      TiingoCandle[] response = getTiingoCandles(portfolioTrade, endDate);
      
      if (response != null) {
        TiingoCandle lastDayResponse = response[response.length - 1];
        totalReturnsDtoList.add(new TotalReturnsDto(ticker, lastDayResponse.getClose()));
      } else {
        throw new RuntimeException();
      }
    }
    return totalReturnsDtoList;
  }

  public static TiingoCandle[] getTiingoCandles(PortfolioTrade portfolioTrade, 
          String endDate) throws URISyntaxException {
    ...
    TiingoCandle[] response = restTemplate.getForObject(uri, TiingoCandle[].class);
    return response;
  }
  ...
  public static List<AnnualizedReturn> mainCalculateSingleReturn(String[] args) 
    ...
    for (PortfolioTrade portfolioTrade : portfolioTrades) {
      ...
      TiingoCandle[] tiingoCandles = getTiingoCandles(portfolioTrade, endDate);
      ...
      if (tiingoCandles != null) {
        ...
        //                         .filter(candle -> candle.getDate().equals(localEndDate) 
        //                           || candle.getDate().equals(localEndDate.minusDays(1)))
        //                         .findFirst().get()
        //                         .getClose();
        ...
        annualizedReturnList.add(
              calculateAnnualizedReturns(localEndDate, 
